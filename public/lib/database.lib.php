...
class DB {
	...
	/**
	 * Permet de préparer et executer une requete SQL de manipulation de données et, en cas d'erreur, d'en récupérer les informations.
	 * @param string $sql : Requête SQL (autre que SELECT) à executer, avec des points d'interrogation "?" ou des variables nommées ":nomvariable" si besoin de variable dans la requête.
	 * @param array $values : Contient les valeurs à attribuer aux inconnues de la requête SQL. En cas d'utilisation des "?", le tableau contient simplement les valeurs sans index particulier.
